...
class LLMSplitter(BaseSplitter):
    """
    基于 LLM 的智能拆分器（委托给 LLMService）。

    保留该类作为 create_splitter / 旧测试的兼容入口，内部不再维护独立的
    prompt、重试与解析逻辑。
    ...
    def split(self, text: str) -> list[str]:
        return self._service.split_text(text, self.max_length)

    def _parse_output(self, content: str) -> list[str]:
        """兼容旧调用，实际解析已在 LLMService 中完成。"""
...
class HybridSplitter(BaseSplitter):
    ...
    def split(self, text: str) -> list[str]:
        ...
        如果 LLM 可用且成功，则使用 LLM 结果，并设置 used_llm=True；
        否则回退规则拆分，used_llm=False。
        """
        self.used_llm = False
        if self._llm is not None:
            try:
                result = self._llm.split(text)
                self.used_llm = True
                return result
